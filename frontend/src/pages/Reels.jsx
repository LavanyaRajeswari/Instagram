...
import { getPosts, repostPost } from "../api/postsApi";
import { getReels as getPaginatedReels } from "../api/reelsApi";
import { useReelsQuery } from "../api/queryHooks";
...
const getHeartClassName = (liked) =>
  `h-4 w-4 ${
    liked ? "fill-[#ed4956] stroke-[#ed4956] text-[#ed4956]" : "text-secondary"
  }`;
const reportOptions = {
  main: {
...
function Reels() {
  ...
  const [repostCountMap, setRepostCountMap] = useState({});
  ...
  const firstReelsQuery = useReelsQuery({ page: 0, size: REELS_PAGE_SIZE });
  ...
  const activeReelCommentsRestricted = hasCommentsDisabled(activeReel);
  const showActiveReelRepost = activeReel && CURRENT_USER_ID != null && !isOwnedByUser(activeReel, CURRENT_USER_ID);

  useEffect(() => {
    if (firstReelsQuery.isLoading) {
      setLoading(true);
      return;
    }
    if (firstReelsQuery.error) {
      loadReels(0, true);
      return;
    }
    if (firstReelsQuery.data) {
      const pagePosts = getPostsFromPageResponse(firstReelsQuery.data);
      appendUniquePosts(pagePosts, true);
      reelPageRef.current = 0;
      setHasMore(!isLastPageResponse(firstReelsQuery.data, pagePosts));
      setLoading(false);
    }
  }, [firstReelsQuery.data, firstReelsQuery.error, firstReelsQuery.isLoading]);
  ...
  useEffect(() => {
    ...
    const newReels = reels.filter((r) => !loadedReelIds.current.has(r.id));
    if (newReels.length === 0) return;
    newReels.forEach((r) => loadedReelIds.current.add(r.id));
    loadReelData(newReels);
  }, [reels.length]);
  ...
  }, [commentsOpen, reportOpen, sharePost, embedOpen, aboutOpen]);
      ...
      reelsNeedingData.map(async (reel) => {
        try {
          ...
          const [count, status, savedStatus] = await Promise.all([
            ...
            CURRENT_USER_ID ? isPostLiked(reel.id) : false,
            CURRENT_USER_ID ? isPostSaved(reel.id) : false,
          ]);
  ...
  const canManageComment = (item) =>
    isOwnedByCurrentUser(item) || isOwnedByUser(activeReel, CURRENT_USER_ID);
  ...
  const handleLike = async (reel) => {
    ...
    try {
      ...
      if (alreadyLiked) await unlikePost(reel.id);
  ...
  const handleSaveToggle = async (reel) => {
    ...
    try {
      ...
      if (alreadySaved) await unsavePost(reel.id);
      else await savePost(reel.id);
  ...
  const openComments = async (reel) => {
    if (hasCommentsDisabled(reel)) return;
  ...
  const handleAddComment = async () => {
    ...
    if (!text || !activeReel || activeReelCommentsRestricted) return;
    if (!CURRENT_USER_ID) {
      return;
    }
    ...
    try {
      if (replyingTo) {
        await addReply(activeReel.id, replyingTo.id, text);
        setExpandedReplies((prev) => ({ ...prev, [replyingTo.id]: true }));
      } else {
        await addComment(activeReel.id, text);
      }
      ...
      if (activeReelCommentsRestricted) return;
      setCommentText(text);
    }
  };
  ...
  const handleDeleteComment = async (event) => {
    ...
    try {
      await deleteComment(activeReel.id, commentOptions.id);
      setCommentsMap((prev) => ({
        ...prev,
  ...
  const handleRepost = () => {
    if (!CURRENT_USER_ID || reposting || !activeReel || isOwnedByUser(activeReel, CURRENT_USER_ID)) return;
  ...
  const confirmRepost = async () => {
    ...
    try {
      setReposting(true);
      await repostPost(activeReel.id);
      const nextCount = (repostCountMap[activeReel.id] || activeReel.repostCount || 0) + 1;
      setRepostCountMap((prev) => ({ ...prev, [activeReel.id]: nextCount }));
      setRepostConfirmOpen(false);
      setCopyToast("Reposted to your profile");
      setTimeout(() => setCopyToast(false), 2000);
    } catch (error) {
      setCopyToast(error?.response?.data?.message || "Could not repost this reel");
      setTimeout(() => setCopyToast(false), 2000);
    } finally {
      setReposting(false);
    }
      ...
      >
        <div className="relative flex w-full min-w-0 items-start gap-2 pr-8">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-1 flex flex-nowrap items-center gap-x-3 whitespace-nowrap text-[11px] leading-4 text-secondary">
              ...
              <button
                ...
                disabled={activeReelCommentsRestricted}
              >
                Reply
              </button>
                ...
                {!isExpanded ? (
                  ...
                  <div className="min-w-0">
                    <button
                      ...
                      className="mb-2 text-[12px] font-semibold text-secondary"
                    >
                      Hide replies
                    </button>
            ...
            <div className="mt-3">
              {!isExpanded ? (
                ...
                <div className="relative min-w-0 pl-5">
                  <span className="absolute bottom-0 left-0 top-0 w-px bg-secondary" />
  ...
  if (loading) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-card">
        <p className="text-sm text-secondary">Loading reels...</p>
      </main>
    );
  }
  ...
  if (reels.length === 0) {
    return (
      <main className="flex min-h-screen flex-col items-center justify-center bg-card">
        <p className="text-sm text-secondary">No reels available yet.</p>
      </main>
    );
  }
  ...
  return (
    <main className="relative min-h-screen bg-card">
      ...
      <section className="mx-auto flex min-h-screen max-w-[900px] items-center justify-center px-4 py-5">
        <div className="relative flex items-end gap-5">
          <div className="relative h-[95vh] max-h-[920px] w-[480px] overflow-hidden rounded-lg bg-black max-sm:w-[calc(100vw-32px)]">
          ...
          {activeReel && (
            <div className="mb-8 flex flex-col items-center gap-5 text-primary">
              <button
                type="button"
              ...
              <button
                ...
                title={activeReelCommentsRestricted ? "Comments are restricted" : undefined}
              >
              ...
              <button type="button" onClick={() => setSharePost({ ...activeReel, itemType: "REEL" })}>
                <Send className="h-8 w-8" />
              </button>

              {showActiveReelRepost && (
                <button
                  type="button"
                ...
                >
                  ...
                  <span className="text-[11px] font-semibold">
                    {repostCountMap[activeReel.id] ?? activeReel.repostCount ?? 0}
                  </span>
                </button>
              )}
              ...
              <div className="relative" ref={moreMenuRef}>
                ...
                {moreMenuOpen && (
                  <div className="absolute bottom-10 left-1/2 z-[100] w-[250px] -translate-x-1/2 overflow-hidden rounded-2xl bg-card text-sm shadow-2xl">
                    <button
                      type="button"
                      onClick={openReportModal}
                      className="block w-full px-5 py-4 text-left font-semibold text-[#ed4956] hover:bg-secondary"
                    >
                      Report
                    </button>
                    <button
                      ...
                      className="block w-full border-t border-secondary px-5 py-4 text-left hover:bg-secondary"
                    >
                      Go to post
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                        setSharePost({ ...activeReel, itemType: "REEL" });
                        setMoreMenuOpen(false);
                      }}
                      className="block w-full border-t border-secondary px-5 py-4 text-left hover:bg-secondary"
                    >
      ...
      {commentsOpen && activeReel && (
        <div className="fixed left-[calc(50%+255px)] top-[54%] z-[200] flex h-[560px] w-[390px] -translate-y-1/2 flex-col overflow-hidden rounded-2xl border border-primary bg-card shadow-2xl max-xl:left-auto max-xl:right-8 max-lg:right-5 max-sm:bottom-0 max-sm:left-0 max-sm:right-0 max-sm:top-auto max-sm:h-[70vh] max-sm:w-full max-sm:translate-y-0 max-sm:rounded-t-2xl">
          <div className="relative flex items-center justify-center border-b border-secondary px-4 py-4">
            <button
              type="button"
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4">
            {activeReelCommentsRestricted ? (
              <p className="mt-10 text-center text-sm font-semibold text-secondary">
          ...
          {replyingTo && (
            <div className="border-t border-secondary px-4 py-2 text-xs text-secondary">
              Replying to{" "}
          ...
          {currentUser && !activeReelCommentsRestricted && (
            <div className="flex items-center gap-2 border-t border-secondary px-3 py-3">
              <img
              ...
              <div className="relative flex flex-1 items-center rounded-full border border-primary px-3 py-2">
                <input
                  ref={commentInputRef}
                  value={commentText}
                ...
                {emojiOpen && (
                  <div
                    ref={emojiPickerRef}
                    className="absolute bottom-10 right-0 z-[500] overflow-hidden rounded-xl border border-primary bg-card shadow-2xl"
      ...
      {loadingMore && (
        <div className="fixed bottom-8 left-1/2 z-40 -translate-x-1/2 rounded-full bg-card px-4 py-2 text-xs font-semibold text-secondary shadow">
          Loading more reels...
        </div>
      )}
      ...
      <div className="fixed right-8 top-1/2 z-40 flex -translate-y-1/2 flex-col gap-5">
        {activeIndex > 0 && (
          <button
            type="button"
            onClick={goPrevious}
            className="flex h-11 w-11 items-center justify-center rounded-full bg-card shadow"
          >
            <ChevronUp />
          </button>
        )}
      ...
      {commentOptions && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/60">
          <div className="w-[320px] overflow-hidden rounded-xl bg-card text-center">
            <button
              type="button"
            ...
            <button
              ...
              className="block w-full border-t py-4 text-secondary"
            >
              Cancel
            </button>
          </div>
      ...
      {copyToast && (
        <div className="fixed left-1/2 top-20 z-[500] -translate-x-1/2 rounded-md bg-tertiary px-5 py-2 text-xs font-semibold text-white">
          {copyToast}
        </div>
      )}
...
{reportOpen && (
  <div className="fixed inset-0 z-[700] flex items-center justify-center bg-black/70 px-4">
    <div className="w-full max-w-[448px] overflow-hidden rounded-2xl bg-card shadow-2xl">
      <div className="relative flex h-[52px] items-center justify-center border-b border-primary">
        ...
        {reportStep === "success" ? null : (
          <h2 className="text-sm font-bold text-primary">Report</h2>
        )}
            ...
            <div className="py-1">
              {(reportOptions[reportStep]?.options || []).map((option) => (
                <button
                  ...
                  className="flex w-full items-center justify-between border-b border-secondary px-4 py-4 text-left text-xs text-primary hover:bg-secondary"
                >
...
{aboutOpen && activeReel && (
  <div className="fixed inset-0 z-[750] flex items-center justify-center bg-black/70 px-4">
    <div className="w-full max-w-[420px] overflow-hidden rounded-2xl bg-card text-primary shadow-2xl">
            ...
            <div className="p-4">
              ...
              <div className="overflow-hidden rounded-lg border border-primary">
                <div className="flex items-center gap-3 p-3">
                  ...
                  />
                  <div className="min-w-0">
                    <p className="truncate text-sm font-semibold text-primary">{activeReel.user?.username || ""}</p>
                    <p className="truncate text-xs text-secondary">{activeReel.user?.fullName || ""}</p>
                  </div>
                </div>

                <div className="flex max-h-[320px] items-center justify-center bg-black">
                  <video src={activeReel.reelVideoUrl} muted playsInline className="max-h-[320px] w-full object-contain" />
                </div>

                {activeReel.caption && (
                  <p className="p-3 text-sm leading-5 text-primary [overflow-wrap:anywhere] [word-break:break-word]">
                    <span className="font-semibold">{activeReel.user?.username || ""}</span> <LinkedText text={activeReel.caption} />
                  </p>
                )}
              </div>
            </div>
