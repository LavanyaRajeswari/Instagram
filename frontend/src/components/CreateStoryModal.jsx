...
function CreateStoryModal({ onClose, onStoryCreated }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true);
      audioRef.current?.pause();
      const story = await createStory({ userId: currentUserId, caption, media, closeFriendsOnly });
      ...
      <div className="fixed inset-0 z-[900] flex items-center justify-center bg-black/70 px-4">
        <div className="w-full max-w-[460px] overflow-hidden rounded-xl bg-card">
          ...
          <div className="max-h-[85vh] overflow-y-auto p-4">
            ...
            <div className="mt-4">
              <p className="text-sm font-semibold">Story Visibility</p>
              <div className="mt-3 space-y-2 text-sm">
                <label className="flex items-center gap-2">
                  <input
                      ...
                      onChange={() => setCloseFriendsOnly(false)}
                  />
                  <span>Public</span>
                </label>
                <label className="flex items-center gap-2">
                  <input
                      ...
                      name="story-visibility"
                      checked={closeFriendsOnly}
                      onChange={() => setCloseFriendsOnly(true)}
                  />
